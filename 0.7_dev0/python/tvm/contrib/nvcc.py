...
def compile_cuda(code,
    ...
    if arch is None:
        ...
        # Find the lowest architecture level
        while True:
            if not nd.gpu(n).exist: break
            if n > 0:
                new_arch = "".join(nd.gpu(n).compute_version.split('.'))
                if new_arch < lowest_arch:
                    lowest_arch = new_arch
            else:
                lowest_arch = "".join(nd.gpu(0).compute_version.split('.')) 
            n += 1
        
        if n > 0:
            arch = "sm_" + lowest_arch
        else:
